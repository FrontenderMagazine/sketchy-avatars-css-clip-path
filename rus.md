...
Примечание от Криса Койера, автора блога, в котором была размещена статья:
...
встретить элементы в рисованном стиле. Очень сложно добиться ощущения рисунка 
без использования множества изображений. Но с помощью простой тригонометрии и очень 
базовых знаний CSS-свойства `clip-path`, мы сможем запросто достигнуть этого и, 
к тому же, без вреда для производительности.
...
сгенерировать множество различных SVG-фигур. Но можно добиться того же самого, 
применив те же математические вычисления для генерации `clip-path` с помощью (S)CSS.
...
сейчас поддерживается, но наверное `clip-path` тоже лучше добавить. Если IE 
или Firefox начнут поддерживать это свойство, оно вероятнее всего будет
беспрефиксным.
...
создания эффекта рисунка.

Вы передаете в `polygon()` список из значений X и Y для вершин, вот так: 
`<x0> <y0>, <x1> <y1>, … <xn> <yn>`. Это отрисует фигуру по точкам 
**в заданном порядке** и обрежет любой контент **за пределами**  
только что созданной формы.
...
По факту, с достаточным количеством точек, мы можем создать `polygon` со столь 
гладкой формой, что он начнет походить на `circle`.
...
так что нам надо добавить различия. Все что нам нужно, это просто сместить
наши точки в различных направлениях для придания желаемого эффекта. 
Числа в переменных `$lower` и  `$upper` могут быть какими угодно, в
зависимости от результата, который вы хотите получить.

    $w:     120px;   // Общая ширина
     
    @function sketchAvatar() {
      $n: 	  60;     // Количество точек
      $lower: -80;    // Нижний порог
      $upper: 80;     // Верхний порог
...
Мы сделали это! Уникальные, рисованные, аватарки с помощью 
CSS-свойства `clip-path: polygon()`:
